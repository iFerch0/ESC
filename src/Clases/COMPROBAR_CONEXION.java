...
public class COMPROBAR_CONEXION
{
    ...
    public LinkedList listar(Connection conn)
        throws SQLException
    {
        LinkedList searchResults = new LinkedList();
        PreparedStatement stmt = conn.prepareStatement("SELECT * FROM test ORDER BY cm_codigo ASC ");
        COMPROBAR_CONEXION temp;
        for(ResultSet result = stmt.executeQuery(); result.next(); searchResults.add(temp))
        {
            temp = new COMPROBAR_CONEXION();
